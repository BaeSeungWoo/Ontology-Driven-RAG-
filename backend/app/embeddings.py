...
def load_colpali_embeddings(config: Config) -> SentenceTransformer:
    ...
    model = SentenceTransformer(model_name=config.colpali_embedding.colpali_model, device=device)
    model.max_seq_length = config.colpali_embedding.colpali_max_seq_length
    return model
...
# ──────────────────────────────────────────────────────────────────────────────
def save_embedding_meta(config: Config, build_colpali: bool = False) -> None:
    ...
    if build_colpali:
        meta = {
            ...
            "colpali_embedding_model": config.colpali_embedding.colpali_model,
            "colpali_max_seq_length": config.colpali_embedding.colpali_max_seq_length,
            "colpali_base_url": config.get_colpali_base_url()
        }
    else:
        meta = {
            "factory_id": config.id,

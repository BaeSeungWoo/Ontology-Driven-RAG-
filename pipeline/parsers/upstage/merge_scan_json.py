...
def get_page_value(elem: dict[str, Any]) -> int:
    """PDF 페이지 수 반환
    ...
    """
    if "page" in elem:
        return int(elem["page"])
    if "metadata" in elem and "page" in elem["metadata"]:
        return int(elem["metadata"]["page"])
    return 0
...
def merge_one_pdf_json(
    ...
    category_id_counter: defaultdict[str, int] = defaultdict(int)
    ...
    for part_dir in part_dirs:
        ...
        for json_file in json_files:
            ...
            elements.sort(key=get_page_value)
            max_page_in_file = 0
            for element in elements:
                merged_element = dict(element)

                category = merged_element.get("category", "unknown")
                if "id" in merged_element:
                    merged_element["original_id"] = merged_element["id"]
                merged_element["id"] = category_id_counter[category]
                category_id_counter[category] += 1

                if "page" in merged_element:
                    target_container = merged_element
                    original_page = int(merged_element["page"])
                elif "metadata" in merged_element and "page" in merged_element["metadata"]:
                    target_container = merged_element["metadata"]
                    original_page = int(merged_element["metadata"]["page"])
                else:
                    target_container = merged_element
                    original_page = 1
                
                new_page = original_page + current_page_offset
                target_container["page"] = new_page

                if original_page > max_page_in_file:
                    max_page_in_file = original_page

                combined_elements.append(merged_element)

        current_page_offset += max_page_in_file
